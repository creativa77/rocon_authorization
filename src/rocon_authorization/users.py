...
def load_msgs_from_yaml_file(file_path):
    """
      ...
      :param str file_path: file path of a yaml formatted interactions
                            file (ext=.interactions).
...
def load_msgs_from_yaml_resource(resource_name):
    """
      ...
      :param str resource_name: pkg/filename of a yaml formatted users
                                file (ext=.users).
...
class User(object):
    ...
    def __init__(self, msg):
        """
          :param msg: underlying data structure with fields minimally filled
                      via :func:`.load_msgs_from_yaml_resource`.
    ...
    def __eq__(self, other):
        if type(other) is type(self):
            return self.msg.user == other.msg.user and \
                self.msg.role == other.msg.role
        else:
            return False
    ...
    def __str__(self):
        ...
        '''
        s = console.cyan + "  Name" + console.reset + "         : " \
            + console.yellow + "%s" % self.msg.user + console.reset + '\n'  # noqa
        s += console.cyan + "  Role" + console.reset + "  : " + \
            console.yellow + "%s" % self.msg.role + console.reset + '\n'  # noqa
